...
Keep each section to 2 to 3 sentences. Be specific and reference actual zones, percentages, and dollar figures. Do not use markdown headers with # symbols. Use plain section titles. Do not use long dashes or em dashes. Use regular hyphens or commas instead.`
    ...
    try {
      ...
      setReports({
        executive: execData.content[0].text,
        analyst: analData.content[0].text,
      })
      ...
      <html>
        <head>
          ...
          <style>
            ...
            ul { font-size: 13px; margin-bottom: 12px; padding-left: 20px; }
        ...
        <body>
          ...
          <div class="meta">Generated by RehabTrack | June 2026 | Based on Sentinel-2 satellite data | ${tone === 'executive' ? 'Executive version' : 'Analyst version'}</div>
          ${currentReport.split('\n').map(line => line.trim() ? `<p>${line}</p>` : '').join('')}
          <div class="footer">Generated by Claude AI | Grounded in Sentinel-2 NDVI data, DMIRS compliance guidelines, and WA Mining Act 1978 | Not a substitute for formal environmental assessment by a qualified consultant.</div>
        </body>
      </html>
    `)
    ...
    <div className="flex h-full">
      ...
      <div className="w-64 bg-[#161b22] border-r border-[#30363d] p-4 flex flex-col gap-4 flex-shrink-0 overflow-y-auto">
        <div>
          ...
          <div className="text-[9px] text-[#8b949e]">Claude writes both an Executive and Analyst version at once. Switch between them instantly with no need to regenerate.</div>
        </div>
          ...
          <div className="flex-shrink-0 px-6 pt-4 flex items-center gap-3">
            ...
            <div className="ml-auto bg-[#21262d] border border-[#30363d] rounded-full p-0.5 flex gap-0.5">
              <button
                ...
                className={`px-3 py-1 rounded-full text-[9px] transition-colors ${tone === 'executive' ? 'bg-[#1a3a1a] text-[#3fb950]' : 'text-[#484f58]'}`}
              >
                Executive
              </button>
              <button
                onClick={() => setTone('analyst')}
                className={`px-3 py-1 rounded-full text-[9px] transition-colors ${tone === 'analyst' ? 'bg-[#1a3a1a] text-[#3fb950]' : 'text-[#484f58]'}`}
              >
                Analyst
              </button>
            </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          {loading && (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <div className="text-[9px] text-[#484f58]">Executive and Analyst versions | takes about 15 seconds</div>
            </div>
          )}
